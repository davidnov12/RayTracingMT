   ...
   namespace sg
   {
      ...
      public:
         ...
         self_type operator+(int shift) const
         {
            self_type ret{*this};
            if(ret._Ind) {
			   //if (shift > 0) printf("ind %d %d %d\n", ret._Ind[0], ret._Ind[1], ret._Ind[2]);
               ret._Ind += 3 * shift;
		 ...
		 virtual bool operator<(const self_type& rhs) const
		 {
			 if (this->_Ind == nullptr)
				 return this->_triangle.v0 < rhs._triangle.v0;

			 return this->_Ind < rhs._Ind;
		 }
		 ...
		 bool isIndexed() { if (_Ind) return true; return false; }

		 unsigned* getIndices() { return _Ind; }
